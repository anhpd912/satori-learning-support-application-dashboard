...
const MOCK_LESSONS: LessonItem[] = [
    ...
    { id: '2', title: 'Bài 2', vocabCount: 36, grammarCount: 14, status: 'DRAFT' },
    { id: '3', title: 'Bài 3', vocabCount: 36, grammarCount: 16, status: 'OPEN' },
    { id: '4', title: 'Bài 4', vocabCount: 36, grammarCount: 13, status: 'ARCHIVED' },
    { id: '5', title: 'Bài 5', vocabCount: 36, grammarCount: 14, status: 'OPEN' },
];
...
export default function LessonListPage() {
    ...
    const courseId = params?.id as string; // Lấy ID khóa học từ URL

    // Giả sử lấy được tên khóa học để hiển thị trên Breadcrumb
    ...
    const fetchData = async () => {
        setIsLoading(true);
        try {
            // TODO: Gọi API lấy danh sách bài học của khóa học này
            // VD: lessonService.getLessonsByCourseId(courseId, { page: currentPage, ... })
            setTimeout(() => {
                setLessons(MOCK_LESSONS);
                setTotalItems(5); // Theo ảnh là 5 bài
                setIsLoading(false);
            }, 500);
        } catch (error) {
    ...
    useEffect(() => {
        fetchData();
    }, [currentPage, searchTerm, statusFilter, courseId]);
        ...
        },
        {
            ...
            render: (lesson) => (
                <div className="flex items-center justify-end gap-3 text-gray-400">
                    <button 
                        onClick={(e) => { e.stopPropagation(); router.push(`/courses/${courseId}/lessons/${lesson.id}`); }} 
    ...
    return (
        <div className="p-8 bg-gray-50 min-h-screen font-sans w-full">
            ...
            <PageHeader 
                ...
                title="Quản lý nội dung bài học"
            />
            ...
            <div className="flex flex-col sm:flex-row gap-4 mb-6">
                ...
                <button className="px-5 py-2.5 bg-white border border-gray-300 text-gray-700 hover:bg-gray-50 text-sm font-medium rounded-lg shadow-sm transition-all flex items-center justify-center gap-2">
