...
export default function UserDetailFeature({ userId, role = 'MANAGER' }: UserDetailFeatureProps) {
  ...
  const handleDelete = async () => {
      ...
      try {
          ...
          setToast({ message: err.message || 'Lỗi khi xóa người dùng', type: 'error', isVisible: true });
          setIsDeleting(false);
      }
  };
  ...
  if (error || !user) {
    return (
        <div className="p-8 text-center bg-gray-50 min-h-screen">
            ...
            <Link href={basePath}>
                <button className="text-blue-600 hover:underline">Quay lại danh sách</button>
            </Link>
        </div>
    );
  }

  const classEnrollments = (user as any).classEnrollments || [];
  const taughtClasses = (user as any).taughtClasses || [];
  const isTeacher = user!.role === 'TEACHER';
  const isLearner = user!.role === 'LEARNER';
  const shouldShowHistory = isTeacher || isLearner;
  ...
  return (
    <div className="p-8 bg-gray-50 min-h-screen font-sans">
      ...
      <ConfirmModal 
        ...
        title="Xóa tài khoản?"
        message="Hành động này sẽ xóa vĩnh viễn tài khoản người dùng này khỏi hệ thống. Bạn có chắc chắn không?"
      ...
      <div className="space-y-6">
        <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-200 flex items-center gap-6">
            <div className="w-20 h-20 rounded-full overflow-hidden border-2 border-gray-100 bg-gray-200">
                <img src={user.avatarUrl} alt={user.name} className="w-full h-full object-cover" onError={(e) => e.currentTarget.src = 'https://via.placeholder.com/150'} />
            </div>
            <div>
                <div className="flex items-center gap-3 mb-1">
                    <h2 className="text-xl font-bold text-gray-900">{user.name}</h2>
                    <span className={`px-2.5 py-0.5 rounded-full text-xs font-medium border ${user.status === 'ACTIVE' ? 'bg-green-50 text-green-700 border-green-200' : 'bg-gray-100 text-gray-500 border-gray-200'}`}>
                        {user.status}
                    </span>
                </div>
                <p className="text-gray-500 font-medium">{getRoleLabel(user.role)}</p>
            </div>
        </div>
        ...
        {shouldShowHistory && (
            <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-200">
                <h3 className="font-bold text-gray-900 mb-4">
                    {isTeacher ? 'Danh sách lớp đang giảng dạy' : 'Lịch sử học tập và khóa học'}
                </h3>
                            ...
                            <table className="w-full text-left">
                                <thead>
                                    <tr className="border-b border-gray-100 text-gray-500 text-xs uppercase font-semibold">
                                        <th className="py-3 pr-4">Mã lớp dạy</th>
                                        <th className="py-3 px-4">Tên khóa học</th>
                                        <th className="py-3 pl-4">Trạng thái lớp</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-gray-50">
                                    {taughtClasses.map((item: any, index: number) => (
                                        <tr key={index}>
                                            <td className="py-3 pr-4 font-medium text-gray-900">
                                                {item.code || item.classId || 'N/A'} 
                                            </td>
                                            <td className="py-3 px-4 text-gray-600">
                                                {item.courseName || item.courseId || 'Chưa cập nhật'}
                                            </td>
                                            <td className="py-3 pl-4">
                                                <span className={`px-2 py-1 rounded text-xs font-medium ${
                                                    item.status === 'ACTIVE' ? 'bg-blue-50 text-blue-700' : 'bg-gray-100 text-gray-600'
                                                }`}>
                                                    {item.status || 'Active'}
                                                </span>
                                            </td>
                    ...
                    classEnrollments.length > 0 ? (
                        <div className="overflow-x-auto">
                            <table className="w-full text-left">
                                <thead>
                                    <tr className="border-b border-gray-100 text-gray-500 text-xs uppercase font-semibold">
                                        <th className="py-3 pr-4">Mã lớp học</th>
                                        <th className="py-3 px-4">Ngày tham gia</th>
                                        <th className="py-3 pl-4">Trạng thái</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-gray-50">
                                    {classEnrollments.map((item: any, index: number) => (
                                        <tr key={index}>
                                            <td className="py-3 pr-4 font-medium text-gray-900">
                                                {item.classId || item.classCode || 'N/A'}
                                            </td>
                                            <td className="py-3 px-4 text-gray-600">
                                                {formatDate(item.createdAt || item.enrollmentDate)}
                                            </td>
                                            <td className="py-3 pl-4">
                                                <span className={`px-2 py-1 rounded text-xs font-medium ${
