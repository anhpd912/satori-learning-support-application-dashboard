...
import { ScenarioDetail, Mission } from '../types';
...
export default function ScenarioDetailClient({ topicId, scenarioId }: ScenarioDetailProps) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            setIsLoading(true);
            try {
                const scenarioRes = await topicService.getScenarioById(scenarioId);
    ...
    // Helper to sort missions if needed
    const sortedMissions = scenario?.missions 
        ? [...scenario.missions].sort((a, b) => (a.orderIndex || a.order || 0) - (b.orderIndex || b.order || 0))
        : [];
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            alert('Có lỗi xảy ra khi xóa bài hội thoại.');
        }
    };
            ...
            <div className="max-w-7xl mx-auto px-8 py-8 space-y-6">
                ...
                <PageHeader 
                    breadcrumb={[
                        ...
                        { label: scenario?.themeName || 'Chi tiết Chủ đề', href: `/topics/${topicId}` },
                        ...
                        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-8 pb-10 relative overflow-hidden group">
                            <div className="absolute top-0 right-0 p-8 flex items-center gap-4">
                                ...
                                <button 
                                    ...
                                    className="flex items-center gap-2 text-red-500 text-sm font-bold hover:opacity-70 transition-opacity"
                                >
                                    Xóa hội thoại
                            ...
                            <div className="flex items-center gap-4 mb-4">
                                <div>
                                    ...
                                    {scenario.titleJapanese && (
                                        <div className="text-sm text-slate-400 font-medium mt-1">{scenario.titleJapanese}</div>
                                    )}
                                </div>
                                <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider border ${
                                    scenario.status === 'PUBLISHED' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-slate-50 text-slate-400 border-slate-100'
                                }`}>
                                    {scenario.status === 'PUBLISHED' ? 'Đang hoạt động' : 'Bản nháp'}
                                    {scenario.status === 'PUBLISHED' && <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></span>}
                                </span>
                            </div>

                            <p className="text-slate-500 leading-relaxed font-normal max-w-4xl">
                                {scenario.descriptionVi || scenario.description}
                            </p>
                            ...
                            <div className="flex items-center gap-6 mt-6">
                                <div className="flex items-center gap-2 px-3 py-1.5 bg-orange-50 text-orange-600 rounded-lg border border-orange-100">
                                    <span className="text-[10px] font-bold uppercase tracking-wider italic">Trình độ: {scenario.jlptLevel || 'N5'}</span>
                                </div>
                                <div className="flex items-center gap-2 px-3 py-1.5 bg-blue-50 text-blue-600 rounded-lg border border-blue-100">
                                    <span className="text-[10px] font-bold uppercase tracking-wider">Độ khó: {scenario.difficultyScore || 0}/10</span>
                                </div>
                                {scenario.courseName && (
                                    <div className="flex items-center gap-2 px-3 py-1.5 bg-slate-50 text-slate-500 rounded-lg border border-slate-100">
                                        <span className="text-[10px] font-bold uppercase tracking-wider">Khóa học: {scenario.courseName}</span>
                                    </div>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                            ...
                            <div className="p-8 space-y-8">
                                <div className="grid grid-cols-2 gap-6">
                                    <div className="flex items-center gap-4 p-6 bg-slate-50/50 rounded-2xl border border-slate-50 group hover:bg-white hover:border-blue-100 hover:shadow-md transition-all">
                                        ...
                                        <div>
                                            ...
                                            <div className="text-base font-bold text-slate-800 tracking-tight">{scenario.learnerRole || 'Người dùng'}</div>
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-4 p-6 bg-slate-50/50 rounded-2xl border border-slate-50 group hover:bg-white hover:border-purple-100 hover:shadow-md transition-all">
                                        ...
                                        <div>
                                            ...
                                            <div className="text-base font-bold text-slate-800 tracking-tight">{scenario.aiRole || 'Hệ thống AI'}</div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div>
                                    ...
                                    <div className="p-6 bg-slate-50/80 rounded-2xl border border-slate-100 text-slate-600 font-normal leading-relaxed">
                                        {scenario.roleplayContext && scenario.roleplayContext.startsWith('"')
                                            ? JSON.parse(scenario.roleplayContext)
                                            : (scenario.roleplayContext || scenario.scenario || scenario.context)}
                                    </div>
                                </div>
                            </div>
                        </div>
                            ...
                            <div className="p-8">
                                ...
                                <div className="space-y-4">
                                    {sortedMissions.map((mission) => (
                                        <div key={mission.id} className="flex items-center gap-4 p-5 bg-white border border-slate-100 rounded-2xl hover:border-blue-100 hover:shadow-sm transition-all group">
                                            ...
                                            <div className="flex flex-col">
                                                <span className="text-base font-semibold text-slate-700">{mission.title}</span>
                                                {mission.description && <span className="text-xs text-slate-400 font-normal">{mission.description}</span>}
                                            </div>
                                        </div>
                                    ))}
                                    {sortedMissions.length === 0 && (
                                        <div className="text-center py-10 text-slate-400 italic font-normal">
